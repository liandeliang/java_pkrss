...
public class StringUtil {
     ...
     * @param newChar
     *            替换后字符
     * @return <li>待替换字符串为null或长度为零时，返回待替换字符串</li> <li>其它，返回替换后结果</li>
     */
    ...
    public static Date strToDate(String datestr) {
        ...
        patterns = (new String[] { "yyyy-MM-dd HH:mm:ss", "yyyy:MM:dd HH:mm", "yyyy:MM:dd HH:mm:ss",
                "yyyy-MM-dd HH:mm", "dd.MM.yy HH:mm", "yyyyMMdd HHmmss", "yyyyMMdd HHmm", "MM/dd/yy hh:mm a",
                "HH:mm:ss dd.MM.yyyy", "yyyy:MM:dd", "yyyy-MM-dd", "dd.MM.yy", "yyyyMMdd", "MM/dd/yy",
                "yyyy:MM:dd HH:mm:sss", "yyyy/MM/dd" });
     ...
     * "91", "92", "93", "94", "95", "96", "97", "98", "99", "9A", "9B", "9C", "9D", "9E", "9F", "A0", "A1", "A2", "A3",
     * "A4", "A5", "A6", "A7", "A8", "A9", "AA", "AB", "AC", "AD", "AE", "AF", "B0", "B1", "B2", "B3", "B4", "B5", "B6",
     * "B7", "B8", "B9", "BA", "BB", "BC", "BD", "BE", "BF", "C0", "C1", "C2", "C3", "C4", "C5", "C6", "C7", "C8", "C9",
     * "CA", "CB", "CC", "CD", "CE", "CF", "D0", "D1", "D2", "D3", "D4", "D5", "D6", "D7", "D8", "D9", "DA", "DB", "DC",
     ...
     * 
     * public static String escape(String s) { StringBuffer sbuf = new StringBuffer(); int len = s.length(); for (int i
     * = 0; i < len; i++) { int ch = s.charAt(i); if ('A' <= ch && ch <= 'Z') { sbuf.append((char) ch); } else if ('a'
     * <= ch && ch <= 'z') { sbuf.append((char) ch); } else if ('0' <= ch && ch <= '9') { sbuf.append((char) ch); } else
     * if (ch == '-' || ch == '_' || ch == '.' || ch == '!' || ch == '~' || ch == '*' || ch == '\'' || ch == '(' || ch
     * == ')') { sbuf.append((char) ch); } else if (ch <= 0x007F) { sbuf.append('%'); sbuf.append(hex[ch]); } else {
     * sbuf.append('%'); sbuf.append('u'); sbuf.append(hex[(ch >>> 8)]); sbuf.append(hex[(0x00FF & ch)]); } } return
     * sbuf.toString(); }
     * 
     * public static String unescape(String s) { StringBuffer sbuf = new StringBuffer(); int i = 0; int len =
     * s.length(); while (i < len) { int ch = s.charAt(i); if ('A' <= ch && ch <= 'Z') { sbuf.append((char) ch); } else
     * if ('a' <= ch && ch <= 'z') { sbuf.append((char) ch); } else if ('0' <= ch && ch <= '9') { sbuf.append((char)
     * ch); } else if (ch == '-' || ch == '_'|| ch == '.' || ch == '!' || ch == '~' || ch == '*'|| ch == '\'' || ch ==
     * '(' || ch == ')') { sbuf.append((char) ch); } else if (ch == '%') { int cint = 0; if ('u' != s.charAt(i + 1)) {
     * cint = (cint << 4) | val[s.charAt(i + 1)]; cint = (cint << 4) | val[s.charAt(i + 2)]; i += 2; } else { cint =
     * (cint << 4) | val[s.charAt(i + 2)]; cint = (cint << 4) | val[s.charAt(i + 3)]; cint = (cint << 4) |
     * val[s.charAt(i + 4)]; cint = (cint << 4) | val[s.charAt(i + 5)]; i += 5; } sbuf.append((char) cint); } else {
     * sbuf.append((char) ch); } i++; } return sbuf.toString(); }
     */
